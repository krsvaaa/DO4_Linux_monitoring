...
Пример запуска - **./main.sh 1**


## 06

Работает с логами по пути **../04/log_file_*.log**. Запускается через **main.sh** без аргументов. Создает **report.html** при помощи GoAccess, который открывается в браузере и показывает аналитику по логам.
